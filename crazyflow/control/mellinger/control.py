...
from crazyflow.control.core import controllable, load_function_params
...
class MellingerStateData:
    ...
    def create(
        ...
        params = load_function_params(state2attitude, drone, xp=jnp, device=device)
...
class MellingerAttitudeData:
    ...
    def create(
        ...
        steps = -jnp.ones((n_worlds, 1), dtype=jnp.int32, device=device)
        params = load_function_params(attitude2force_torque, drone, xp=jnp, device=device)
...
class MellingerBodyRateData:
    ...
    def create(
        ...
        zeros_4d = jnp.zeros((n_worlds, n_drones, 4), device=device)
        steps = -jnp.ones((n_worlds, 1), dtype=jnp.int32, device=device)
        params = load_function_params(body_rate2force_torque, drone, xp=jnp, device=device)
...
class MellingerForceTorqueData:
    ...
    def create(
        ...
        zeros_4d = jnp.zeros((n_worlds, n_drones, 4), device=device)
        steps = -jnp.ones((n_worlds, 1), dtype=jnp.int32, device=device)
        params = load_function_params(force_torque2rotor_vel, drone, xp=jnp, device=device)
