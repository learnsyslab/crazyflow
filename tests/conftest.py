...
def vectorize(*arrays: Array | None) -> Array:
    """Concatenate arrays along their last axis into one vector, skipping ``None`` entries.

    Used to compare e.g. all derivatives returned by a dynamics function or all states of a
    ``SimData`` at once.
    """
    arrays = [x for x in arrays if x is not None]
    return array_namespace(*arrays).concat(arrays, axis=-1)
